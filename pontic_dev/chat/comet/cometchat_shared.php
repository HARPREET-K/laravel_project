...
function sql_getQuery($key, $params = array()){
	...
	$sql = $sql_queries[$key];
	$excludeEscapeforKeys = array('querystring', 'timestampCondition', 'condition', 'prepend', 'prependCondition', 'guestpart', 'originalsql', 'extra', 'sqlpart', 'insertvalues', 'limitClause', 'usertable', 'set', 'userids');
	if(!empty($params)){
	...
	if (!empty($to) && isset($message) && $message!='' && $userid > 0) {
		...
		if (USE_COMET == 1) {
			...
			if($dir <> 3) {
				...
				$localmessageid = !empty($localmessageid)? $localmessageid: (!empty($_REQUEST['callback'])) ? $_REQUEST['callback'] : '';
